...
<head>
	...
	<title>Detail Lab | Sistem Informasi Praktek Kerja Industri</title>
			...
			<div class="sidebar-overlay" id="sidebar-overlay"></div>
                ...
                <article class="content item-editor-page">
                    <div class="title-block">
                        <h3 class="title"> Detail Laboratorium <?php//harusnya nama lab?>
                            <span class="sparkline bar" data-type="bar"></span>
                        </h3>
						<p class="title-description"><?php echo $laboratorium->desk_lab;?></p>
                    </div>	
                    <?php $attributes = array("name" => "uplabform", "id"=>"uplabform");
				echo form_open_multipart('HakKelola/prosesEditLab', $attributes);?>
                        <div class="card card-block">						
						<input name="id_lab" type="hidden" value="<?php echo $laboratorium->id_lab; ?>" />						
						<div class="title-block">
						<label class="title">Edit Data Laboratorium</label>
						</div>
                            <div class="form-group row">
                                <label class="col-sm-2 form-control-label text-xs-right"> Nama Laboratorium: </label>
                                <div class="col-sm-10">
                                    <input type="text" class="form-control boxed" placeholder="Edit Nama Laboratorium" name="nama" value="<?php echo $laboratorium->nama_lab;?>" ></div>
                            </div>
							<div class="form-group row">
                                <label class="col-sm-2 form-control-label text-xs-right"> Deskripsi Laboratorium: </label>
                                <div class="col-sm-10">
                                    <input type="text" class="form-control boxed" placeholder="Edit Deskripsi Laboratorium" name="desk_lab" value="<?php echo $laboratorium->desk_lab;?>"></div>
                            </div>
							<div class="form-group row">
                                <label class="col-sm-2 form-control-label text-xs-right"> Kapasitas Maksimum: </label>
                                <div class="col-sm-10">
                                    <input type="text" class="form-control boxed" placeholder="Edit Kapasitas Maksimum" name="maks" value="<?php echo $laboratorium->kapasitas_maksimum;?>" ></div>
                            </div>							
							<div class="form-group row">
					...
					<div class="card card-block">						
					<div class="title-block">
						<label class="title">Daftar Siswa</label>
						</div>
						...
						<table class="table table-striped">
							<thead>
								...
								<!-- <a href="<?php echo site_url('HakKelola/akhiriPrakerin/' .$lab->id_siswa);?>" >Akhiri</a> -->
							</tr>
							<tbody>
							<?php foreach($isiLab->result() as $iL):?>
							<?php $i = $i+1;?>
								<tr>
									<td><?php echo $i; ?></td>
									<td><?php echo $iL->nama_siswa;?></td>
									<td><?php  echo $iL->jurusan_siswa;?></td>
									<td><?php  echo $iL->nama_sekolah;?></td>
									<td>
									<?php if(!empty($iL->id_pemb_lab)){ ?>									
										<?php foreach ($iLab->result() as $npemb){?>									
										<?php if($iL->id_pemb_lab == $npemb->id_pemb_lab){?>
											<?php  echo $npemb->nama_pemb_lab;?>
										<?php } ?>
										<?php } ?>
									<?php }else{?>
									<p> -- </p>
									...
									<td><?php echo $iL->tanggal_mulai_praktek;?> - <?php  echo $iL->tanggal_selesai_praktek;?>
									</td>
									<td><?php  if($iL->id_akun == 0 && empty($iL->kode_referal)){ ?>
											<p>Belum Diproses</p>
										<?php }else if($iL->id_akun != 0 && !empty($iL->kode_referal)){ ?>
											<p>Belum Aktivasi</p>
										<?php }else { ?>
											<p>Sudah Aktivasi</p>
										<?php } ?>
					...
					<?php }else{?>
									<br/><center><h1>Tidak ada Siswa yang bekerja di lab.</h1></center>
						<?php }?>
					</div>
					</section>
				</article>
